...
在 PHP 7 之后，PHP 7.4 版本将于 2019 年，11 月 28 号发布。没有 alpha 或者 beta 版本，将直接以市面上的通用版本发布。

该版本的新特性当中最令人激动的部分使得 [PHP 网站开发][1] 更加可靠和快速了。

尽管早已得知 PHP 8 对于 PHP 而言将会是一个真正的里程碑版本，但是其距离发布还有相当长的时间。

接下来我们将目光聚集于一些你将会使用到的 PHP 7.4 新特性。
...
如果传值是可为空类型你可以使用 "null" 来作为默认值。当然，这个理解起来很简单，但是默认为空的参数也带来了一些意料之外的影响，其中就包括了下面这个例子：
...
幸运的是，这种特殊情况的传值是不被允许的。对于 "object" 类型或者 "class" 类型也一样，这样是不可能获取默认值的。如果需要设置这些属性的默认值，你必须要使用构造函数。使用构造函数来初始化传值是很直观的：
...
然而，你需要确保在构造函数之外为未初始化的属性赋值是有效的。初始化检查将在读取属性的时候执行。
...
虽然 "$bar" 被实例化，但它的值不是整型，因此 "$bar" 被访问时 PHP 将报告一个错误信息：
...
如你所见，多了一种"状态变量"叫未初始化。如果 "$bar" 没有赋值，那么它的值就是 "null"。这使得不管是确认设置的属性是否可为空或者遗忘掉赋值都不可能。这是因为 PHP 7.4 将引入新的变量 "uninitialised"。

### 让我们看一下关于未初始化变量的一些关键点：

* 读取未初始化的属性将会显示一个致命错误信息。
* 如果属性的类型不可为空，你可以在创建对象的时候标明为未初始化属性，当访问该属性的时候将被校验。
* 在读取之前，你可以声明一个未初始化的属性。
* unset 掉一个无类型的属性将值设置为 "null" 但是 unset 掉一个已声明的类型属性，值将会成为 "uninitialised"。

你可以从下面的代码得知：在创建对象之后为可以为不可为空和未初始化的属性赋值：
...
在赋值的时候已经完成了类型检查，只有在读取属性值的时候，才会分析是否是未初始化的状态。这样确保了最终属性的值不会是无效的类型。
...
[新版本的 php][2] 在执行代码之前会在后台将当前代码中的 "self" 替换成子类。解决办法如下所示：
...
但是，新版本的 PHP 7.4 中改变类型不允许将类型从可为空转变为不可为空，反之亦然。
...
让我们来看看可以传入什么类型以及以何种方式传入。需要确保传入的属性在当前类下是可行的。为了标记可行性，需要在它们前面加上访问修饰符或者关键字 “var”。
...
在这里，"callable" 是一个私有的 "Bar::method"，虽然根据上下文它是指 "Foo"。由于这个问题，PHP 7.4 并没有支持 "callable"。

然而，这并不是什么大问题，因为 PHP 早已引入了"闭包"，所以根据上下文当需要的时候将调用 "$this" 即可。
...
OK，这些就是你将会在 PHP 7.4 通用版本中看到的一些重要属性类型了。你可以联系我们获知更多关于 [PHP web 开发服务][4] 的信息
